...
describe('ApprovalGate', () => {
  ...
  it('escalates after repeated approval denials for the same tool', async () => {
    ...
    expect(first.verdict).toBe('denied');
    expect(first.reason).toBe('needs_user');
    expect(first.consecutiveDenials).toBe(1);
    expect(second.verdict).toBe('denied');
    expect(second.escalated).toBe(true);
    expect(second.reason).toBe('needs_user');
    expect(second.denialMessage).toContain('Stop retrying');
  });
  ...
  it('resets denial tracking after an approved run', async () => {
    ...
    expect(deniedAgain.verdict).toBe('denied');
    expect(deniedAgain.reason).toBe('needs_user');
